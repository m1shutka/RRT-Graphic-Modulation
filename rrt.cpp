...
Point Steer(const Point& a, const Point& b)
{
	...
	while (!CollisionFree(a, z) and z.x < 800 and z.x > 0)
	{
		z.x -= dx;
		z.y -= dy;
	}
	if(z.x > 0 and z.x < 800){ return z; }
	else { return a; }
}
...
vector<Point> RRT(int S, Graph& g, const Point& Qinit, const Point& Qgoal, const vector<pair<Point, float>>& v)
{
	...
	vector<Point> result;
	auto parents = shortest_path(g, 0, g.points.size() - 1);

	double min_path_length = 800 * 800;
	int nearest_to_goal = N;
	for (int i = 0; i < g.points.size(); ++i)
	{
		if (length(g, build_path(parents, 0, i)) + Qgoal.distance_p(g.points[i]) < min_path_length and CollisionFree(g.points[i], Qgoal)) {
			min_path_length = length(g, build_path(parents, 0, i)) + Qgoal.distance_p(g.points[i]);
			nearest_to_goal = i;
		}
	}

	if (nearest_to_goal == N) {
		return result;
	}

	auto path = build_path(parents, 0, nearest_to_goal);

	for (int i = 0; i < path.size(); ++i)
	{
		result.push_back(g.points[path[i]]);
	}
	result.push_back(Qgoal);
	g.add_vertex(Qgoal);
	g.add_edge(g.points[nearest_to_goal], Qgoal, Qgoal.distance_p(g.points[nearest_to_goal]));
	return result;
}